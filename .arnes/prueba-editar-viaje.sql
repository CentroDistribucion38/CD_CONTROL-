...
begin
  select id into v_id from public.traspaso_registrar(
    (public.traspaso_hoy() - 5),'A','pet','ABC 123','ag01','planta',1);

  -- 1. UN SUPERVISOR NO CORRIGE. Registra, anula lo suyo, pero no reescribe.
  begin
    perform public.traspaso_editar_viaje(v_id, (public.traspaso_hoy() - 5),'A','pet','XXX999','ag01','planta',1);
...
begin
  select id into v_id from public.traspaso_registrar(
    (public.traspaso_hoy() - 4),'A','pet','ABC 123','ag01','planta',1,false,null,null,'la nota');

  -- 3. Corrige placa, tipo, ruta, turno, cantidad y fecha de un golpe.
  perform public.traspaso_editar_viaje(
    v_id, (public.traspaso_hoy() - 3),'B','casco','bhg-156','planta','galapa',3,false,null,null,null,
  ...
  if (v_j->>'fecha')::date <> (public.traspaso_hoy() - 3) then
    v_falla := v_falla || ' 3(fecha)'; end if;
  ...
  begin
    perform public.traspaso_editar_viaje(v_id, (public.traspaso_hoy() - 3),'B','casco','BHG156','ag01','ag01',1);
  ...
  begin
    perform public.traspaso_editar_viaje(v_id, (public.traspaso_hoy() - 3),'Z','casco','BHG156','ag01','planta',1);
    v_falla := v_falla || ' 6b(acepto turno Z)';
  exception when others then
    if sqlerrm not like '%A, B o C%' then v_falla := v_falla || ' 6b(' || sqlerrm || ')'; end if;
  end;
  begin
    perform public.traspaso_editar_viaje(v_id, (public.traspaso_hoy() - 3),'B','noexiste','BHG156','ag01','planta',1);
  ...
  begin
    perform public.traspaso_editar_viaje(v_id, (public.traspaso_hoy() - 3),'B','casco','BHG156','ag01','planta',0);
    v_falla := v_falla || ' 6d(acepto cero viajes)';
  exception when others then
    if sqlerrm not like '%al menos un viaje%' then v_falla := v_falla || ' 6d(' || sqlerrm || ')'; end if;
  end;
  begin
    perform public.traspaso_editar_viaje(v_id, (public.traspaso_hoy() - 3),'B','casco','','ag01','planta',1);
  ...
  perform public.traspaso_editar_viaje(v_id, (public.traspaso_hoy() - 3),'B',null,null,null,null,2,true);
  select to_jsonb(v) into v_j from public.traspasos_viajes v where v.id = v_id;
  ...
  perform public.traspaso_editar_viaje(v_id, (public.traspaso_hoy() - 3),'B','pet','ABC123','ag01','planta',1,false);
  ...
  begin
    perform public.traspaso_editar_viaje(v_id, (public.traspaso_hoy() - 3),'B','pet','ABC123','ag01','planta',1);
  ...
  begin
    perform public.traspaso_editar_viaje(gen_random_uuid(), (public.traspaso_hoy() - 3),'B','pet','ABC123','ag01','planta',1);
  ...
  begin
    ...
    select coalesce(sum(c.cumplido),0) into v_c0 from public.v_traspasos_control c
      where c.fecha = (public.traspaso_hoy() - 2) and c.turno = 'C';
    select coalesce(sum(c.cumplido),0) into v_a0 from public.v_traspasos_control c
      where c.fecha = (public.traspaso_hoy() - 2) and c.turno = 'A';

    select id into v2 from public.traspaso_registrar(
      (public.traspaso_hoy() - 2),'A','pet','ABC123','ag01','planta',1);
    perform public.traspaso_editar_viaje(v2, (public.traspaso_hoy() - 2),'C','pet','ABC123','ag01','planta',1);
