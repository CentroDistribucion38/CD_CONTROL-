#!/usr/bin/env bash
# =====================================================================
# UN VIAJE CON VARIOS TIPOS, contra Postgres de verdad.
# SE CORRE SIN AGRUPAR NADA, como lo corre el editor de Supabase.
#   bash .arnes/correr-traspasos-varios-tipos.sh
# =====================================================================
set -e
DB=${1:-tp_vt}
...
insert into public.traspasos_tipos (clave, nombre, activo, orden) values
  ('casco','Casco vidrio',true,2), ('estibas','Estibas',true,3)
on conflict (clave) do update set activo = true;
...
$PSQL -d $DB -f supabase/migraciones/2026-09-traspasos-varios-tipos.sql 2>&1 | grep -E "NOTICE|ERROR" || true
echo "--- segunda vuelta"
$PSQL -d $DB -f supabase/migraciones/2026-09-traspasos-varios-tipos.sql 2>&1 | grep -E "ERROR" && exit 1

echo "--- las pruebas"
salida=$($PSQL -d $DB -f .arnes/prueba-traspasos-varios-tipos.sql 2>&1) || true; echo "$salida" | grep -E "NOTICE|ERROR" || true
