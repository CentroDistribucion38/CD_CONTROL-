...
# LAS MIGRACIONES DE ANTES. Se toleran los fallos a propósito: alguna
# puede depender de un módulo que esta base no monta (el maestro de
# conteos toca Acciones y Roturas), y lo que se está probando aquí es
# otra cosa. Lo que NO se tolera es un fallo de la migración nueva.
echo "--- las migraciones de antes"
for f in supabase/migraciones/2026-09-conteo-estibas-y-saldo.sql ; do
  if [ -f "$f" ]; then $PSQL -d $DB -f "$f" >/dev/null 2>&1 || true; fi
done

echo "--- la migración, dos veces (tiene que poder correrse varias veces)"
$PSQL -d $DB -f supabase/migraciones/2026-09-conteo-preanotacion.sql 2>&1 | grep -E "NOTICE|ERROR" || true
echo "--- segunda vuelta"
$PSQL -d $DB -f supabase/migraciones/2026-09-conteo-preanotacion.sql 2>&1 | grep -E "^ERROR|psql:.*ERROR" && exit 1

echo "--- las pruebas"
$PSQL -d $DB -f .arnes/prueba-conteo-preanotacion.sql 2>&1 | grep -E "NOTICE|ERROR"
