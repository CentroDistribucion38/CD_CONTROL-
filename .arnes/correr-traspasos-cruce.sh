...
$PSQL -d $DB -f supabase/migraciones/2026-09-traspasos-cruce-sap.sql 2>&1 | grep -E "NOTICE|ERROR" || true
echo "--- segunda vuelta"
$PSQL -d $DB -f supabase/migraciones/2026-09-traspasos-cruce-sap.sql 2>&1 | grep -E "ERROR" && exit 1

echo "--- las pruebas"
salida=$($PSQL -d $DB -f .arnes/prueba-traspasos-cruce.sql 2>&1) || true; echo "$salida" | grep -E "NOTICE|ERROR" || true
