...
set -e
export DB=${1:-vbeasy}
export EXCLUIR="visto-bueno-easy|descargo-del-ol"
...
$PSQL -d $DB -f supabase/migraciones/2026-09-roturas-visto-bueno-easy.sql 2>&1 | grep -E "ERROR" || true
if $PSQL -d $DB -f supabase/migraciones/2026-09-roturas-visto-bueno-easy.sql 2>&1 | grep -E "^ERROR|ERROR:"; then exit 1; fi
echo "--- las pruebas"
salida=$($PSQL -d $DB -f .arnes/prueba-visto-bueno-easy.sql 2>&1) || true; echo "$salida" | grep -E "NOTICE|ERROR" || true
