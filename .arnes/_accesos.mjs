...
// src/modulos/admin/libro-accesos.ts
var import_qrcode_generator = __toESM(require_qrcode());
import ExcelJS from "exceljs";

// src/modulos/admin/colores-rol.ts
var MARCA = { tinta: "12263A", banda: "FFC000" };
var VIVOS = ["00B050", "0A84FF", "8B3DFF", "00B8A9", "FF6A00", "FF2D78", "E0A800", "00A2E8"];
function colorRol(clave, roles, c = MARCA) {
...
var BLANCO = "FFFFFFFF";
var VERDE = "FF00B050";
var GRIS_FIJO = "FF8A8E8A";
function paleta(c) {
  const ok = (x) => !!x && /^[0-9a-f]{6}$/i.test(x);
  const t = ok(c?.tinta) ? c.tinta : MARCA.tinta, b = ok(c?.banda) ? c.banda : MARCA.banda;
  return {
    TINTA: aHex(hex(t)),
    BANDA: aHex(hex(b)),
    GRIS: aclarar(t, 0.64),
    LINEA: aclarar(t, 0.13),
    FONDO: aclarar(t, 0.045),
    PAGINA: aclarar(t, 0.09),
    SUAVE: aclarar(b, 0.22),
    HONDO: oscurecer(b, 0.45)
  };
}
var relleno = (argb) => ({ type: "pattern", pattern: "solid", fgColor: { argb } });
var letra = (size, color, bold = false, extra = {}) => ({ name: "Calibri", size, bold, color: { argb: color }, ...extra });
var MONO = "Consolas";
var EMU = 9525;
var en = (col, dx, fila, dy) => ({ nativeCol: col, nativeColOff: Math.round(dx * EMU), nativeRow: fila, nativeRowOff: Math.round(dy * EMU) });
var pxCol = (ancho) => Math.trunc(ancho * 7 + 5);
var pxFila = (pt) => pt * 4 / 3;
var hoy = (d) => d.toLocaleDateString("es-CO", { day: "2-digit", month: "2-digit", year: "numeric" });
function qr(texto) {
...
function cabecera(h, P, sello, titulo, sub, ancho) {
  h.views = [{ showGridLines: false }];
  h.getRow(1).height = 6;
  for (let c = 1; c <= ancho; c++) h.getCell(1, c).fill = relleno(P.BANDA);
  h.getRow(2).height = 9.75;
  h.getRow(3).height = 30;
  h.getRow(4).height = 16;
  h.getRow(5).height = 9.75;
  if (sello != null) h.addImage(sello, { tl: en(1, 4, 2, 0), ext: { width: 40, height: 40 } });
  const t = h.getCell(3, 3);
  t.value = titulo;
  t.font = letra(18, P.TINTA, true);
  t.alignment = { vertical: "middle" };
  const s = h.getCell(4, 3);
  s.value = sub;
  s.font = letra(9.5, P.GRIS);
}
async function armarPases(o) {
  ...
  const cuando = o.emitido ?? /* @__PURE__ */ new Date();
  const corto = cuando.toLocaleDateString("es-CO", { day: "2-digit", month: "2-digit", year: "2-digit" }).replace(/\//g, "\xB7");
  ...
  cabecera(L, P, sello, "Accesos a CONTROL", `Respaldo para quien entrega los pases \xB7 no se reparte \xB7 ${o.pases.length} ${o.pases.length === 1 ? "persona" : "personas"} \xB7 ${hoy(cuando)}`, 7);
  const cab = ["Nombre", "Usuario", "Clave provisional", "Rol", "Entregado a la persona"];
  L.getRow(6).height = 24;
  cab.forEach((t, i) => {
    const c = L.getCell(6, 2 + i);
    c.value = t;
  ...
  o.pases.forEach((p, i) => {
    const f = 7 + i, r = L.getRow(f);
    r.height = 24;
    const rc = colorRol(p.rol, o.roles, C);
    const vals = [
      [p.nombre, letra(10.5, P.TINTA, true)],
      [p.usuario, letra(10.5, P.TINTA, false, { name: MONO })],
      [p.clave, letra(13, P.TINTA, true, { name: MONO })],
      [p.rolNombre.toUpperCase(), letra(9, "FF" + rc.letra, true)],
      ["\u2610  s\xED   \xB7   fecha: ______", letra(9.5, P.GRIS)]
    ];
    vals.forEach(([v, fu], k) => {
    ...
    r.getCell(4).fill = relleno(P.BANDA);
    r.getCell(4).numFmt = "@";
  ...
  nota.value = "Las claves van como texto para no perder los ceros de la izquierda. Si corriges una aqu\xED, su pase se actualiza.";
  ...
  H.views = [{ showGridLines: false }];
  const pintar = (f, c1, c2, argb) => {
    for (let c = c1; c <= c2; c++) H.getCell(f, c).fill = relleno(argb);
  };
  const unir = (f1, c1, f2, c2) => H.mergeCells(f1, c1, f2, c2);
  const pon = (f, c, v, fu, al = {}) => {
    const x = H.getCell(f, c);
    x.value = v;
    x.font = fu;
    x.alignment = { vertical: "middle", ...al };
    return x;
  };
  H.getRow(1).height = 7.5;
  H.getRow(2).height = 30;
  H.getRow(3).height = 18;
  H.getRow(4).height = 13.5;
  pon(2, 2, "Pases de acceso a CONTROL", letra(16, P.TINTA, true));
  pon(3, 2, `${o.pases.length} ${o.pases.length === 1 ? "persona" : "personas"}  \xB7  emitidos el ${hoy(cuando)}  \xB7  imprime, recorta por la l\xEDnea punteada y entrega cada pase en la mano`, letra(9.5, P.GRIS));
  ...
  o.pases.forEach((p, i) => {
    const r0 = 5 + i * 9, fl = 7 + i;
    ALTOS.forEach((a, k) => H.getRow(r0 + k).height = a);
    const rc = colorRol(p.rol, o.roles, C);
    for (let f = r0; f <= r0 + 7; f++) {
      pintar(f, 2, 7, BLANCO);
      pintar(f, 9, 10, BLANCO);
    }
    pintar(r0, 2, 5, P.BANDA);
    pintar(r0 + 1, 2, 5, P.BANDA);
    const franja = pxFila(ALTOS[0] + ALTOS[1]);
    if (sello != null) H.addImage(sello, { tl: en(1, 10, r0 - 1, (franja - 38) / 2), ext: { width: 38, height: 38 } });
    unir(r0, 3, r0, 5);
    pon(r0, 3, "CONTROL", letra(16, P.TINTA, true), { vertical: "bottom" });
    unir(r0 + 1, 3, r0 + 1, 5);
    pon(r0 + 1, 3, `PASE DE ACCESO \xB7 ${o.lugar.toUpperCase()}`, letra(8.5, P.HONDO, true), { vertical: "top" });
    unir(r0, 6, r0 + 1, 7);
    ...
    [["NOMBRE", 2, 3], ["USUARIO", 4, 5], ["EMITIDO", 6, 7]].forEach(([t, a, b]) => {
      unir(r0 + 3, a, r0 + 3, b);
      pon(r0 + 3, a, t, letra(7.5, GRIS_FIJO, true), { indent: 1, vertical: "bottom" });
      unir(r0 + 4, a, r0 + 4, b);
    });
    ...
    cl.numFmt = "@";
    unir(r0 + 6, 6, r0 + 7, 7);
    pon(r0 + 6, 6, "V\xE1lida hasta el primer ingreso. Ah\xED la cambias por una tuya.", letra(8, P.GRIS), { wrapText: true, vertical: "top" });
    ...
    unir(r0 + 6, 9, r0 + 6, 10);
    pon(r0 + 6, 9, "ESCANEA Y ENTRA", letra(9, P.TINTA, true), { horizontal: "center", vertical: "bottom" });
    unir(r0 + 7, 9, r0 + 7, 10);
    pon(r0 + 7, 9, host, letra(7.5, P.GRIS), { horizontal: "center", vertical: "top" });
...
async function armarUsuarios(o) {
  const P = paleta(o.colores), C = { tinta: P.TINTA.slice(2), banda: P.BANDA.slice(2) };
  const wb = new ExcelJS.Workbook();
  wb.creator = "CONTROL \xB7 Usuarios";
  wb.created = /* @__PURE__ */ new Date();
  ...
  const cifras = [
    ...
    ["CLAVE PROVISIONAL", prov, prov ? "FFFF6A00" : VERDE],
    ["NUNCA HAN ENTRADO", nunca, nunca ? "FFFF2D78" : VERDE]
  ];
  h.getRow(6).height = 15;
  h.getRow(7).height = 30;
  h.getRow(8).height = 12;
  const pares = [[2, 2], [3, 4], [5, 7], [8, 9]];
  cifras.forEach(([t, v, raya], i) => {
    const [a, b] = pares[i];
    if (b > a) {
      h.mergeCells(6, a, 6, b);
      h.mergeCells(7, a, 7, b);
    }
    for (let c = a; c <= b; c++) {
      h.getCell(6, c).fill = relleno(P.FONDO);
      h.getCell(7, c).fill = relleno(P.FONDO);
    }
    const lado = { left: { style: "thick", color: { argb: raya } }, right: { style: "thick", color: { argb: BLANCO } } };
    const x = h.getCell(6, a);
    x.value = t;
    x.font = letra(7.5, P.GRIS, true);
    x.alignment = { indent: 1, vertical: "bottom" };
    x.border = lado;
    const y = h.getCell(7, a);
    y.value = v;
    y.font = letra(20, P.TINTA, true);
    y.alignment = { indent: 1, vertical: "middle", horizontal: "left" };
    y.border = lado;
  });
  const F0 = 9;
  h.getRow(F0).height = 24;
  COL.forEach((t, i) => {
    const c = h.getCell(F0, 2 + i);
    c.value = t;
    c.font = letra(9, BLANCO, true);
    c.fill = relleno(P.TINTA);
  ...
  g.forEach((p, i) => {
    ...
    r.getCell(4).fill = relleno("FF" + rc.fondo);
    r.getCell(4).font = letra(9, "FF" + rc.letra, true);
    r.getCell(5).fill = relleno(p.activo ? VERDE : "FF8A8E8A");
    r.getCell(5).font = letra(9, BLANCO, true);
    if (p.provisional && p.activo) r.getCell(6).font = letra(10, "FFFF6A00", true);
    r.getCell(7).numFmt = "dd/mm/yyyy hh:mm";
  ...
  const CR = ["Rol", "Qu\xE9 hace", "Personas", "Activas", "Pantallas"];
  R.getRow(6).height = 24;
  CR.forEach((t, i) => {
    const c = R.getCell(6, 2 + i);
    c.value = t;
    c.font = letra(9, BLANCO, true);
    c.fill = relleno(P.TINTA);
    c.alignment = { vertical: "middle", indent: 1 };
  ...
  o.roles.forEach((rl, i) => {
    const f = 7 + i, r = R.getRow(f);
    r.height = 24;
    const rc = colorRol(rl.clave, o.roles, C);
    const de = g.filter((p) => p.rol === rl.clave);
    [rl.nombre.toUpperCase(), rl.manda ? "Administra la plataforma: entra a todo." : rl.descripcion ?? "", de.length, de.filter((p) => p.activo).length, rl.pantallas].forEach((v, k) => {
      const c = r.getCell(2 + k);
      c.value = v;
      c.font = letra(10, P.TINTA, false);
      c.alignment = { vertical: "middle", indent: 1, wrapText: k === 1, horizontal: k === 0 ? "center" : k >= 2 ? "right" : "left" };
      c.border = { bottom: { style: "thin", color: { argb: P.LINEA } } };
    });
    r.getCell(2).fill = relleno("FF" + rc.fondo);
    r.getCell(2).font = letra(9.5, "FF" + rc.letra, true);
  });
