...
#
#   bash .arnes/correr-inventario-base.sh
# =====================================================================
set -e
DB=${1:-inv_base}
...
$PSQL -d $DB -f .arnes/siembra-permisos-base.sql 2>&1 | grep -E "ERROR" && exit 1
...
# /inventario/base ya estuviera abierta y la migración no hiciera falta.
# ---------------------------------------------------------------------
...
begin
  select count(*) into v_n from public.rol_permisos
   where seccion = '/inventario/base' and rol <> 'yatiene';
  if v_n > 0 then
    raise notice 'OJO: % rol(es) YA tenían la base antes de la migración. La migración no prueba nada.', v_n;
  else
    raise notice 'Bien: hoy nadie ve /inventario/base salvo el que la tiene a mano. Es lo que esta migración viene a abrir.';
  end if;
end $$;
SQL

echo "--- la migración, dos veces (tiene que poder correrse varias veces)"
$PSQL -d $DB -f supabase/migraciones/2026-09-inventario-base.sql 2>&1 | grep -E "NOTICE|ERROR" || true
echo "--- segunda vuelta"
$PSQL -d $DB -f supabase/migraciones/2026-09-inventario-base.sql 2>&1 | grep -E "ERROR" && exit 1

echo "--- las pruebas"
salida=$($PSQL -d $DB -f .arnes/prueba-inventario-base.sql 2>&1) || true; echo "$salida" | grep -E "NOTICE|ERROR" || true
