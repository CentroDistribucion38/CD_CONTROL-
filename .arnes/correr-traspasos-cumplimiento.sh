#!/usr/bin/env bash
# =====================================================================
# QUE CUENTA EN EL CUMPLIMIENTO (estibas solo de Arenosa, tolvas fuera),
# contra Postgres de verdad.
# SE CORRE SIN AGRUPAR NADA, como lo corre el editor de Supabase.
#   bash .arnes/correr-traspasos-cumplimiento.sh
# =====================================================================
set -e
DB=${1:-tp_cum}
...
for f in .arnes/supabase-local.sql \
         ...
         supabase/migraciones/2026-09-traspasos-varios-tipos.sql ; do
...
insert into public.traspasos_tipos (clave, nombre, activo, orden) values
  ('casco','Casco vidrio',true,2), ('estibas','Estibas',true,3), ('tolvas_vidrio','Tolvas de Vidrio',true,10)
on conflict (clave) do update set activo = true;
...
$PSQL -d $DB -f supabase/migraciones/2026-09-traspasos-cumplimiento.sql 2>&1 | grep -E "NOTICE|ERROR" || true
echo "--- segunda vuelta"
$PSQL -d $DB -f supabase/migraciones/2026-09-traspasos-cumplimiento.sql 2>&1 | grep -E "ERROR" && exit 1

echo "--- las pruebas"
salida=$($PSQL -d $DB -f .arnes/prueba-traspasos-cumplimiento.sql 2>&1) || true; echo "$salida" | grep -E "NOTICE|ERROR" || true
