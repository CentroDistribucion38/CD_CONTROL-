...
set -e
export DB=${1:-averias}
export EXCLUIR="2026-09-averias"
...
$PSQL -d $DB -f supabase/migraciones/2026-09-averias.sql 2>&1 | grep -E "ERROR" || true
if $PSQL -d $DB -f supabase/migraciones/2026-09-averias.sql 2>&1 | grep -E "^ERROR|ERROR:"; then exit 1; fi
echo "--- las pruebas"
salida=$($PSQL -d $DB -f .arnes/prueba-averias.sql 2>&1) || true; echo "$salida" | grep -E "NOTICE|ERROR" || true
